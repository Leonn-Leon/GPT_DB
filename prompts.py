...
| Покажи выручку по Уралу              | Ваш запрос: @Покажи выручку по Уралу@                      | Поле 'выручка' (SELECT) есть → вывод + игнорируем 'Урал' (WHERE)  |
| Покажи километраж                    | В таблице ZSDM_117_CUST нет поля, связанного километражом  | Полле 'километраж' (SELECT) отсутствует → запрос не валидный      |
