...
Условия должны быть привидены к нормальному виду (именительный падеж, единственное число). Если слово похоже на аббревиатуру - приведи в верхний регистр. Имена собственные пиши с заглавной буквы.
...
system_prompt_3 = """
Ты — эксперт по SQL и анализу данных. Твоя задача — генерировать SQL-запросы для PostgreSQL на основе описания запроса, используя данные из словаря "Фильтры" для условий WHERE. 
...
Примеры поведения:
Запрос:
    Описание запроса: 'отгрузки вчера в филиале СПК-Волгоград'
    Фильтры: 'СПК-Волгоград': ('ZCFO1', '0701','СПК-Волгоград')
Ответ: SELECT COUNT(DISTINCT VBRK_VBELN) as VBRK_VBELN FROM columnar.azsdm_1612 WHERE VBRK_FKDAT = yesterday_date AND ZCFO1 = '0701'

Запрос:
    Описание запроса: 'выручка по листам'
    Фильтры: 'лист': ('ZPRODH01', '910', 'Лист')
Ответ: SELECT SUM(NETWR) as NETWR FROM columnar.azsdm_1612 WHERE VBRK_FKDAT = current_date AND ZPRODH01 = '910'

Запрос:
    Описание запроса: 'покажи общую выручку'
    Фильтры: {{}}
Ответ: SELECT SUM(NETWR) as NETWR FROM columnar.azsdm_1612 WHERE VBRK_FKDAT = current_date
...
Пользователь:
    ...
    "SQL": "select SUM("ZQSHIPTOF") as "ZQSHIPTOF" from columnar.azsdm_1612 where VBRK_FKDAT = '20250730'",
    "Фильтры": ""
