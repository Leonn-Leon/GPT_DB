...
Правила:
1. На вход будет подан запрос от пользователя, сгенерированный SQL по этому запросу и фильтры.
2. Фильтры нужны, что бы получить информацию для ограничений из запроса. 
3. Струкрутра фильтров: "условие ограничения: (название поля, код, текст)"
4. В комментарий необходимо добавить описание названия поля (его бери в структуре таблицы) и текст из фильтров.
5. Используй поля из select sql-запроса, обёрнутые в скобки <>.
6. Не используй переносы строк в комментарии.
